    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div class="max-w-md mx-auto">
                <form action="<?php echo BASE_URL; ?>/urunler.php" method="GET" class="relative">
                    <input type="text" name="arama" 
                           ...
                           class="w-full pl-12 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <i class="fas fa-search text-gray-400"></i>
                    </div>
                    <button type="submit" class="absolute inset-y-0 right-0 pr-3 flex items-center text-red-600 hover:text-red-700">
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center mb-12 animate-on-scroll">
            <a href="<?php echo BASE_URL; ?>" 
               class="bg-red-600 text-white px-8 py-3 rounded-lg hover:bg-red-700 transition duration-300 font-semibold">
                <i class="fas fa-home mr-2"></i>Ana Sayfaya Dön
            </a>
            <a href="<?php echo BASE_URL; ?>/urunler.php" 
               class="border-2 border-red-600 text-red-600 px-8 py-3 rounded-lg hover:bg-red-600 hover:text-white transition duration-300 font-semibold">
        ...
        <!-- Popular Categories -->
        <div class="animate-on-scroll">
            <h2 class="text-xl font-semibold text-black mb-6">Popüler Kategoriler</h2>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <?php foreach ($categories as $category): ?>
                    <a href="<?php echo BASE_URL; ?>/kategori/<?php echo $category['slug']; ?>" 
                       class="group bg-white rounded-lg p-4 shadow-md hover:shadow-lg transition duration-300 border border-gray-200 hover:border-red-300">
                        <?php if ($category['image']): ?>
                            <img src="<?php echo $category['image']; ?>" 
                                 alt="<?php echo htmlspecialchars($category['name']); ?>" 
                                 class="w-12 h-12 object-cover rounded-lg mx-auto mb-3">
                        <?php else: ?>
                            <div class="w-12 h-12 bg-red-100 rounded-lg flex items-center justify-center mx-auto mb-3 group-hover:bg-red-200 transition duration-300">
                                <i class="fas fa-tools text-red-600"></i>
                            </div>
                        <?php endif; ?>
                        
                        <h3 class="text-sm font-semibold text-black text-center group-hover:text-red-600 transition duration-300">
        ...
        <div class="mt-12 animate-on-scroll">
            <div class="bg-red-50 rounded-lg p-6 border border-red-200">
                <h3 class="text-lg font-semibold text-red-900 mb-2">Yardıma mı ihtiyacınız var?</h3>
...
// Get product variants with color codes for recent products
$recentProductVariants = [];
foreach ($recentProducts as $product) {
    $recentProductVariants[$product['id']] = fetchAll("
...
<section class="py-16 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12 animate-on-scroll">
            <h2 class="text-3xl font-bold text-black mb-4">Son Eklenen Ürünler</h2>
            ...
            <?php foreach ($recentProducts as $product): ?>
                <div class="bg-gray-50 rounded-lg overflow-hidden hover:shadow-lg transition duration-300 animate-on-scroll">
                    ...
                    <div class="p-4">
                        ...
                        <?php if (!empty($recentProductVariants[$product['id']])): ?>
                            <div class="mb-3">
                                <div class="flex items-center space-x-1">
                                    <span class="text-xs text-gray-500">Renkler:</span>
                                    <div class="flex space-x-1">
                                        <?php foreach ($recentProductVariants[$product['id']] as $variant): ?>
